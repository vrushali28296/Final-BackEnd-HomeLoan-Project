...
public class DocumentController {
	...
	@PostMapping(value="/savedocuments/{customerid}", consumes=MediaType.MULTIPART_FORM_DATA_VALUE )
	public ResponseEntity<Documents>saveDocuments(@PathVariable int customerid , @RequestPart(required = true , value="addressProof")MultipartFile addressProof,
			...
			) throws IOException 
	{
		...
		c.getDocuments().setPhoto(photo.getBytes());
		c.getDocuments().setSalaryslip(salaryslip.getBytes());
		c.getDocuments().setSignature(signature.getBytes());
		c.getDocuments().setStatus(document);
		cr.save(c);
